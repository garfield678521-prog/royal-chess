...
function runAI() {
  if (mode !== "ai" || game.result || game.turn !== aiColor || aiThinking) return;
  aiThinking = true;
  clearSelection();
  render();

  // Give the browser a frame before searching so the UI can update.
  setTimeout(() => {
    try {
      ...
      console.error("Royal Chess AI error:", err);
      setText("gameStatus", "AI error — try starting a new AI game.");
    } finally {
      aiThinking = false;
      render();
    }
...
function setConnection(conn, color, host) {
  ...
  const fail = msg => {
    console.error(msg);
    setText("roomStatus", msg);
    render();
  };

  conn.on("open", () => {
    setText("roomStatus", `Connected • You are ${names[myColor]}`);
    if (isHost) conn.send({ type: "sync", state: statePayload() });
    render();
  });
  ...
  conn.on("data", data => {
    ...
    } else if (data.type === "sync" || data.type === "reset") {
  ...
  conn.on("close", () => {
    if (peerConnection === conn) peerConnection = null;
    setText("roomStatus", "Opponent disconnected");
    render();
  });

  conn.on("error", err => fail("Connection error — check the room code and internet connection."));
}

function createPeer(id) {
  try {
    return id ? new Peer(id, { reliable: true }) : new Peer({ reliable: true });
  } catch (e) {
    console.error(e);
    return null;
  }
...
function hostMultiplayer() {
  if (typeof Peer === "undefined") {
    alert("Multiplayer could not load its connection service. Refresh the page and try again.");
    return;
  }
  if (peer) peer.destroy();

  mode="multiplayer"; myColor="w"; isHost=true; roomCode=randomRoomCode();
  game.reset(); moveNotations=[]; clearSelection(); flipped=false;
  document.getElementById("multiplayerPanel").classList.remove("hidden");
  setText("roomCode", roomCode); setText("roomStatus", "Creating room…"); render();

  peer=createPeer("royal-chess-"+roomCode.toLowerCase());
  if(!peer){ setText("roomStatus","Could not start multiplayer."); return; }
  ...
  peer.on("connection", conn => {
    if(peerConnection) { conn.close(); return; }
    setConnection(conn,"w",true);
  });
  peer.on("error", err => {
    console.error("PeerJS host error:",err);
    setText("roomStatus", err && err.type === "unavailable-id" ? "Room code already in use — create another room." : "Could not create room. Try again.");
  });
}
...
function joinMultiplayer() {
  ...
  if(!/^[A-Z0-9]{6}$/.test(code)) return alert("Enter the 6-character room code.");
  if(peer) peer.destroy();

  mode="multiplayer"; myColor="b"; isHost=false; roomCode=code;
  game.reset(); moveNotations=[]; clearSelection(); flipped=true;
  document.getElementById("multiplayerPanel").classList.remove("hidden");
  setText("roomCode",code); setText("roomStatus","Connecting to White…"); render();
  ...
  peer.on("error", err => {
    ...
    setText("roomStatus","Could not join room. Check the code and make sure the host is waiting.");
  });
}
